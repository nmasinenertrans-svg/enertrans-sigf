...
import { workOrderStatusLabelMap } from '../services/workOrdersService'
    ...
    >
      ...
      <FormRow label="Estado">
        <select
          ...
          onChange={(event) => onFieldChange('status', event.target.value as WorkOrderStatus)}
        >
          {workOrderStatuses.map((status) => (
            <option key={status} value={status}>
              {workOrderStatusLabelMap[status]}
            </option>
          ))}
        </select>
      </FormRow>
