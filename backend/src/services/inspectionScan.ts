...
IMPORTANTE — no dudes de mas: decidi cada marca o linea UNA sola vez y segui. Si una marca de la tabla CHECK LIST o una linea de OBSERVACIONES no tiene un item CLARAMENTE correspondiente en el catalogo (puede pasar si la planilla usa categorias distintas a las del catalogo, ej. "Niveles", "Estado y existencias", etc.), NO le dediques mas vueltas ni trates de forzarla en el item que mas se le parezca: mandala directo a "unmatchedNotes" con un label corto que incluya la categoria/nombre tal cual figura en la planilla (ej. "Niveles - Refrigerante: con observacion"). Es preferible una nota sin mapear que perder tiempo de mas o forzar un match dudoso. No inventes ni repitas informacion. No dejes afuera ningun dato de esta hoja: cada marca de la tabla y cada linea de OBSERVACIONES tiene que terminar en matchedItems O en unmatchedNotes, nunca perdida.
